...
const insertCustomer = db.prepare('INSERT INTO customers (name, email, country) VALUES (?, ?, ?)');
...
for (let i = 0; i < 30; i++) {
    ...
    let totalAmount = 0;
    const orderInfo = insertOrder.run(customerId, orderDate.toISOString(), 0, status);
    ...
    // Update order total
    db.prepare('UPDATE orders SET total_amount = ? WHERE id = ?').run(totalAmount.toFixed(2), orderId);
}
